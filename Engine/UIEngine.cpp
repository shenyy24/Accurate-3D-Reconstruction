...
void UIEngine::ProcessFrame()
{
    ...
    {
        Matrix3f tmpR;
        inputIMUMeasurement->R.inv(tmpR);
        this->initR = tmpR;
        this->constdt = inputIMUMeasurement->dt;
    }
    //printf("IMUcached %f\n",inputIMUMeasurement->dt);
    inputIMUMeasurement->R = inputIMUMeasurement->R * (this->initR);
    inputIMUMeasurement->dt = (inputIMUMeasurement->dt - (this->constdt));
    //printf("IMU%f,%f\n",inputIMUMeasurement->dt,constdt);
